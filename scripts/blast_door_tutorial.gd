...
@export var MIN_CLOSE_SPEED := 0.5
@export var MAX_CLOSE_SPEED := 2. 
...
func _crystal_destroyed():
	...
	if crystal_count == 1:
		...
		# trigger player shot the crystal
		EventsBus.player_objective_complete_trigger.emit(ZoneNames.id.GATE_CYCLING)
